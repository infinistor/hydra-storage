...
export default class AddLogWindow extends JetView {
	config() {
		const topBar = 48
		...
		return {
			...
			head: {
				view: "toolbar", cols: [
					{ width: 5 },
					{ view: "label", label: "Add Journal Disks", css: "header_label" }
				],
			},
			body: {
				view: "form",
						...
						rows: [
							{
								...
								rows: [
									...
									{
										view: "text", label: "Pool Name", labelWidth: labelWidth, name: "pool_name", id: "addLog_poolName", readonly: true
									},
									{
										height: 15
									},
									...
									{
										view: "treetable", id: "addLog_tree", scroll: "y", autoheight: true, borderless: true, headerRowHeight: 24, columns: [
											{
									...
									},
									{
										...
										rows: [
											...
											{
												view: "label", id: "addLog_noDisks", label: "No disks available", align: "center", hidden: true
											},
											{
												height: 15
											},
											{
												cols: [
													{
														view: "richselect", id: "addLog_configuration", label: "Configuration", labelWidth: labelWidth, readonly: true, options: [
															{ id: "log", value: "Log" },
														], value: "log"
													},
													{
														view: "button", value: "Add", css: "new_style_primary", width: 70, click: function () {
															...
															var selectedDisks = $$("addLog_availableDisks").serialize().filter((disk) => {
															...
															$$("addLog_noGroups").hide()

															// get the configuration
															var configuration = $$("addLog_configuration").getValue()
															...
															var allDisks = $$("addLog_availableDisks").serialize()
															for (var i = 0; i < allDisks.length; i++) {
																for (var j = 0; j < selectedDisks.length; j++) {
																	if (allDisks[i].name == selectedDisks[j].name) {
																		...
																		$$("addLog_availableDisks").updateItem(allDisks[i].id, allDisks[i])
																	}
																}
															}
														}
											...
											},
											{
												...
												onClick: {
													remove_group: function (e, id, trg) {
														...
														if (this.count() == 0) {
															$$("addLog_noGroups").show()
														}
													}
												}
											},
											{
												view: "label", label: "No journal disks added yet", id: "addLog_noGroups", hidden: false, align: "center"
											},
										]
									},
									{
										...
										cols: [
											{
												view: "button", value: "Apply", css: "new_style_primary", width: 70, click: () => {
													...
													if ($$("addLog_groups").serialize().length == 0) {
														// show error message
														webix.message({ type: "error", text: "At least one log disk should be added" })
														return
													}

													$$("add_log_window").disable()
													$$("add_log_window").showProgress({
														type: "icon",
														hide: false,
													})
													...
													var diskGroups = $$("addLog_groups").serialize()
													...
													webix.ajax().post("/api/storage/pools/" + pool_id + "/diskgroups", JSON.stringify(json)).then((response) => {
														...
														$$("pool_details_tree").load("/api/storage/pools/" + pool_id + "/status").then(() => {
															$$("add_log_window").enable()
															$$("add_log_window").hideProgress()
															this.hideWindow()
														}).fail((xhr) => {
															ajaxFail(xhr)
														})
			...
			on: {
				onShow: () => {
					...
					webix.ajax().get("/api/storage/pools/disks").then((response) => {
						$$("addLog_availableDisks").clearAll()
						var data = response.json()
						availableDisks = data
						$$("addLog_availableDisks").parse(availableDisks)
					...
					webix.ajax().get("/api/storage/pools/" + pool_id).then((response) => {
						...
						$$("addLog_poolName").setValue(pool.name)
					})

					$$("addLog_tree").load("/api/storage/pools/" + pool_id + "/status").then(() => {
						// $$("addLog_tree").closeAll()
					})
				},
				onHide: () => {
					$$("addLog_poolName").setValue("")
					$$("addLog_tree").clearAll()

					$$("addLog_configuration").setValue("log")

					$$("addLog_groups").clearAll()
					$$("addLog_noGroups").show()

					$$("add_log_form").clear()
					$$("add_log_form").clearValidation()

					$$("addLog_availableDisks").markSorting()
				}
			},
	...
	init() {
		webix.extend($$("add_log_window"), webix.ProgressBar)
	}
	ready() {
		var networkCreatePoolWindow = $$("add_log_window")
	...
	resizeElement() {
		const AddLogForm = $$("add_log_window")
		...
		if (AddLogForm) {
			...
			AddLogForm.config.height = height - (topBar + labelToolbar)
			AddLogForm.resize()
		}
	}
}