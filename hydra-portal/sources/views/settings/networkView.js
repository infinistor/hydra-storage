...
export default class NetworkView extends JetView {
	config() {
		return {
			...
			rows: [
				{
					type: "header", template: "<span style='font-weight: 100; margin-right: 5px'>Settings</span> <span style='font-weight: 100; margin-right: 5px'>></span> <span>Network</span>",
					borderless: true,
				},
				NetworkTab
			],
	...
	resizeElement() {
		...
		if (networkView) {
			...
			networkView.config.height = height - (topBar + notificationWindowDatatable + notificationBar)
			networkView.resize()
		}
