...
var editPhysicalWindow
export default class PhysicalInterfaceMenu extends JetView {
	config() {
		var options = []
		var edit = this.edit
		...
		return {
			...
			on: {
				...
				onBeforeShow: () => {
					options = []
					var slave = this.getParam("slave", true);
					var status = this.getParam("status", true);

					if (slave === "--") options.push("Edit")
					if (status == "Active") {
						options.push("Deactivate")
					} else {
						options.push("Activate")
					}
					if (slave === "--") options.push("Add VLAN")

					$$("physical_interface_menu").clearAll()
					$$("physical_interface_menu").parse(options)
				}
			}
		}
	}
	init() {
		editPhysicalWindow = this.ui(EditWindow)
	}
	edit() {
		editPhysicalWindow.showWindow()
	}
